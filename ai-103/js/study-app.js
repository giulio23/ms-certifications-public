...
const SUB_TO_SECTION = { '1.1':'1A', '1.2':'1B', '1.3':'1C', '1.4':'1D', '2.1':'2A', '2.2':'2B', '2.3':'2C', '3.1':'3A', '3.2':'3B', '3.3':'3C', '4.1':'4A', '4.2':'4B', '5.1':'5A', '5.2':'5B' };
...
function getAllQuestions() {
  ...
  if (typeof COMBINED_QUESTIONS !== 'undefined') {
    COMBINED_QUESTIONS.forEach(q => {
      if ((q.sourceBank === 'skillcertpro' || q.generated) && q.sub && SUB_TO_SECTION[q.sub]) {
...
function loadProgress() {
  try {
    return JSON.parse(localStorage.getItem('ai103-study-progress') || '{}');
  } catch { return {}; }
}

function saveProgress(data) {
  localStorage.setItem('ai103-study-progress', JSON.stringify(data));
}
