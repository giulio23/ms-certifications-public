...
// 2. copy shared engines + ML study assets (all public-safe) verbatim
for (const f of ['app.js', 'study-app.js', 'learn-app.js', 'copy-util.js', 'mastery.js', 'study-guide.js', 'questions.js', 'study-questions.js']) {
  fs.copyFileSync(path.join(PRIV, 'js', f), path.join(PUB, 'js', f));
}
fs.copyFileSync(path.join(PRIV, 'css/styles.css'), path.join(PUB, 'css/styles.css'));

// 3. copy study.html + learn.html verbatim (no skillcertpro rendering when the bank has none)
